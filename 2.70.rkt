...
(define (encode-symbol symbol tree)
    (cond ((leaf? tree) '())
          ((symbol-in-set? symbol (left-branch tree)) 
           (cons 0 (encode-symbol symbol (left-branch tree))))
          ((symbol-in-set? symbol (right-branch tree)) 
           (cons 1 (encode-symbol symbol (right-branch tree))))
          (else (error "error message"))))

(define (symbol-in-set? symbol tree)
  (not
   (false? (findf 
...
> (define eight-symbols-tree
    (generate-huffman-tree
     '((A 1) (BOOM 1) (GET 2) (JOB 2) (NA 16) (SHA 3) (YIP 9) (WAH 1))))
...
> (encode '(GET) eight-symbols-tree )
;; '(1 1 0 0)
> (encode '(A) eight-symbols-tree )
;; '(1 1 1 1 0)
